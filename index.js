...
function findIndexByProperty(arr, name) {
  ...
  if(!Array.isArray(arr)){// Перевіряємо, чи вхідний параметр є масивом, якщо ні повертаємо -1
    return -1;
  }
...
function findLastEvenNumber(arr) {
  ...
  const foundElement = arr.findLast((element) => element % 2 === 0);// За допомогою метода findLast знаходимо останнє парне число в масиві.

  return foundElement !== undefined ? foundElement : "Парних чисел не знайдено";// Повертаємо знайдене число або 'Парних чисел не знайдено', якщо таке число не знайдено.
}
...
function getSubArrayAndConvertToString(arr, startIdx) {
  ...
  const stringifiedSubArray = subArray.join(" ");// За допомогою методу join() конвертуємо підмасив в рядок.

  return stringifiedSubArray; // Повертаємо рядок.
}
...
function getArrayKeys(arr) {
  ...
  const keysIterator = arr.keys();// За допомогою методу keys() отримуємо об'єкт ітератора, який містить ключі масиву.

  const keysArray = Array.from(keysIterator);// Конвертуємо ітератор в масив за допомогою методу from.

  return keysArray;// Повертаємо масив ключів.
}
...
function getPositiveArrayValues(arr) {
  ...
  const valuesIterator = arr.values();// За допомогою методу values() отримуємо об'єкт ітератора, який містить значення масиву.

  const valuesArray = Array.from(valuesIterator);// Конвертуємо ітератор в масив.
 
  const positiveValuesArray = valuesArray.filter((value) => value > 0); // За допомогою методу filter() отримуємо масив лише з додатніми значеннями.

  return positiveValuesArray;// Повертаємо масив додатніх значень.
}
...
function sumAndPrint(arr) {
  ...
  const sumPrint = arr.reduce((acc, curr) => acc + curr, 0);// За допомогою методу reduce() обчислюємо суму елементів масиву.

  arr.forEach((curr) => console.log(curr));// За допомогою методу forEach() виводимо кожен елемент масиву в консоль.

  return sumPrint;// Повертаємо суму елементів масиву.
}
...
function flattenSquareAndFilter(arr, threshold) {
  ...
  const flattenArray = arr.flat();// За допомогою методу flat() вирівнюємо масив.

  const squareArray = flattenArray.map((value) => value ** 2);// За допомогою методу map() підносимо кожен елемент масиву до квадрату.

  const filterArray = squareArray.filter((arr) => arr > threshold);// За допомогою методу filter() фільтруємо елементи, які більше за порогове значення.

  return filterArray;// Повертаємо вирівняний масив, елементи якого підняті до квадрату і профільтровані.
...
function getLastElementsSumAndPrint(arr, n) {
  ...
  const lastElements = arr.slice(-n);// За допомогою методу slice() вибираємо останні n елементів масиву.

  const sum = lastElements.reduceRight((acc, curr) => acc + curr, 0);// За допомогою методу reduceRight() обчислюємо суму останніх n елементів масиву.

  lastElements.forEach((curr) => console.log(curr)); // За допомогою методу forEach() виводимо кожен елемент з останніх n в консоль.
...
function wordLengths(arr) {
  ...
  const wordLengthsArray = arr.map((word) => word.length);// Використовуємо метод map для створення нового масиву, де кожний елемент - це довжина відповідного слова з вхідного масиву

  return wordLengthsArray;// Повертаємо новий масив
}
...
function insertElementAt(arr, index, element) {
  ...
  return newArr;// Повертаємо масив з вставленим елементом
}
...
function expandArrayByN(arr, n) {
  ...
  if(!Array.isArray(arr)){// Перевіряємо, чи вхідні параметри є масивом та числом відповідно, якщо ні повертаємо 'Перший вхідний параметр має бути масивом, другий - числом'
    return 'Вхідний параметр має бути масивом, другий - числом';
  }
  return arr.flatMap((value) => Array.from({ length: n }, () => value));// Використовуємо метод flatMap для створення нового масиву, де кожне число повторюється n разів
...
/**
  ...
  */
  function findLongestWord(arr) {
    if(!Array.isArray(arr)) {// Перевіряємо, чи вхідний параметр є масивом, якщо ні, повертаємо пустий рядок
      return "";
    }

    let longestWord = arr[0];

    arr.reduce((acc, curr) => {// Використовуємо метод reduce() для знаходження найдовшого слова

    if (curr.length > longestWord.length) {// Порівнюємо довжину поточного слова з довжиною найдовшого слова
      longestWord = curr;
    }
      return longestWord;
    }, longestWord);// Інакше повертаємо попереднє найдовше слово без змін
...
function findLastNegativeNumberIndex(arr) {
  ...
  return arr.findLastIndex((e1) => e1 < 0);
}
